    ...
    if request.user.is_authenticated:
        ...
        if user == "doctor":
            ...
            try:
                ...
                # Fetch all patients under the doctor's care
                patients = PatientData.objects.filter(doctor=doctor_data)
                print(notification)
                notifications = depression_recovery.serializer.NotificationSerializer(notification, many=True)
                print(notifications)
                # Use a Subquery to get the latest Beck's Index for each patient
                latest_becks_index = BecksIndex.objects.filter(patient=OuterRef('pk')).order_by('-timestamp')
                patients_with_latest_index = patients.annotate(
                    latest_becks_score=Subquery(latest_becks_index.values('score')[:1]),
                    latest_becks_timestamp=Subquery(latest_becks_index.values('timestamp')[:1])
                )
            except DoctorData.DoesNotExist:
                patients_with_latest_index = []
                doctor_data = None
                notifications = None
            ...
            return render(request, "pateintHome.html", {'user': patient_data
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                patient = PatientData.objects.create(user=user, doctor=doctor, JoinedTime=now())
...
def GetPatientData(request):
    filter_date = (datetime.now()) #now().date()
    print(filter_date)
    ...
    for x in data:
        try:
            ...
            continue
    print(compute_hourly_max(dataTime))
...
def getVideoMood(request):
    current_date = (datetime.now()) #now().date()
    print(request.data)
    ...
    for x in data:
        try:
            ...
            print(f"Error parsing timestamp: {x[ 'timestamp' ]}. Error: {e}")
            continue
    filterdData = compute_hourly_max(dataTime)
    print(f'Found : {json.dumps(dataTime)}\n')
    print(f'Found : {compute_hourly_max(dataTime)}')
...
def compute_hourly_max(data):
    hourly_data = defaultdict(list)

    # Group data by hour
    for time, value in data:
        hour = int(time)  # Extract the hour part
        hourly_data[hour].append(value)

    # Compute maximum or most common value for each hour
    hourly_max = []
    for hour, values in sorted(hourly_data.items()):
        # Find the most frequent value or the highest
        most_common_value = Counter(values).most_common(1)[0][0]
        hourly_max.append([hour, most_common_value])
...
def sendNote(request):
    try:
        ...
        note = request.data.get('query')
        if not note:
            return Response({"status": "error", "message": "Query cannot be empty."}, status=400)

        # Create the notification
        Notification.objects.create(
            timestamp=now(),
            fromUser=request.user,  # Assuming the current user is sending the note
            toUser=patient_data.doctor.user if patient_data.doctor else None,  # Send to the assigned doctor
            message=note
        )
...
from rest_framework.decorators import api_view
from rest_framework.response import Response
from .models import PatientData, BecksIndex
...
def getBecksScores(request , patient_id):
    try:
        ...
        # Filter BecksIndex entries based on the patient
        becksIndex = BecksIndex.objects.filter(patient=patient_data)

        # Serialize the data
        serializer = BecksIndexSerializer(becksIndex, many=True)
        print('found')
